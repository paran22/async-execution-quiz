...
// 3번째: <blank>`,
    ...
    explanation: {
      ...
      points: [
        '<strong>핵심 ①</strong> — async 함수는 <strong>반환값이 무조건 Promise</strong>. <code>return 42</code>라고 써도 실제로는 <code>Promise&lt;42&gt;</code>가 반환됨',
        '<strong>핵심 ②</strong> — 함수 내부 코드는 <strong>await를 만나기 전까지 동기적으로 실행</strong>됨. 이 문제엔 await가 없으니 <strong>내부 전체가 동기로 실행</strong>되고, 마지막 <code>return</code>에서 값이 Promise로 감싸져 나감',
        '실행 흐름:',
        '&nbsp;&nbsp;• <code>getNumber()</code> 호출 → <code>A</code>가 즉시 찍힘 → <code>Promise.resolve(42)</code> 반환',
        '&nbsp;&nbsp;• <code>.then</code> 콜백은 마이크로태스크 큐로 들어감 → 동기 코드 <code>1</code> 먼저 실행',
        '&nbsp;&nbsp;• 그 다음 큐에서 꺼내져 <code>42</code> 출력',
        '<strong>결과: A → 1 → 42</strong>',
        '<strong>async 없이 같은 동작 만들기</strong> — Promise를 직접 반환하면 됨',
        '&nbsp;&nbsp;<code>function getNumber() { console.log(\'A\'); return Promise.resolve(42); }</code>',
        '두 버전은 <strong>완전히 동일하게 동작</strong>. async는 <code>return</code> 값을 자동으로 <code>Promise.resolve()</code>로 감싸주는 <strong>문법 설탕</strong>일 뿐',
      ],
    },
  },
  {
